  ...
  */
  ...
  addCollectionItemsUsingTags: function(spec, generatedStore, components, options, schemaCache, sourceMap) {
    ...
    _.forEachRight(tagFolders, (tagFolder, tagName) => {
      const sourceMapKey = `#/folders/${encodeURIComponent(tagName)}`;
  ...
  convertChildToItemGroup: function (openapi, child, components, options, schemaCache, variableStore, sourceMap) {
    ...
    if (resource.requestCount > 1) {
      let sourceMapKey = `#/folders/${encodeURIComponent(resource.name)}`;
      if (resource.name === '{id}' && resource.requests.length) {
        const get = resource.requests.find((r) => {
          return r.method === 'get';
        });
        if (get) {
          sourceMapKey = `#/folders/${encodeURIComponent(get.path)}`;
        }
      }
  ...
  convertRequestToItem: function(openapi, operationItem, components, options, schemaCache, variableStore, sourceMap) {
    ...
    if (operation.responses) {
      ...
      _.forOwn(operation.responses, (response, code) => {
        ...
        if (pmBody && pmBody.contentHeader) {
          originalRequestHeaders = _.concat(originalRequestHeaders, pmBody.contentHeader);
        }
